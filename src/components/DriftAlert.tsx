  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        className={`px-8 py-2 rounded-2xl border-2 ${bgColor} ${borderColor} ${textColor} shadow-xl backdrop-blur-2xl border-white/40`}
      >
        <div className="flex items-center gap-4">
          <div className={`p-2 rounded-xl bg-white shadow-sm ${iconColor} shrink-0`}>
            <AlertTriangle className="w-5 h-5" />
          </div>
          <div className="flex-1 min-w-0">
            <h3 className="font-bold text-sm tracking-tight leading-none">Portfolio Drift Detected</h3>
            <p className="text-xs opacity-80 leading-relaxed font-medium">
              {events.length} asset class{events.length > 1 ? 'es' : ''} drifted.
            </p>
          </div>
          <div className="flex items-center gap-1">
            <button onClick={onToggleMinimize} className="p-1.5 hover:bg-black/5 rounded-lg transition-colors">
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {events.map(event => (
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 bg-white hover:bg-slate-50 rounded-lg text-[10px] font-bold uppercase tracking-widest transition-all border border-black/10 shadow-sm active:scale-95"
            >
              <span className="truncate max-w-[100px]">{event.asset_class}</span>
            </button>
          ))}
        </div>
      </motion.div>
